...
class EmailService:
    ...
    def _save_email_to_db(self, message_data: Dict) -> Optional[Email]:
        ...
        try:
            ...
            if existing_email:
                # 기존 이메일 업데이트
                existing_email.subject = message_data.get(
                    "subject", existing_email.subject
                )
                existing_email.sender = message_data.get(
                    "sender", existing_email.sender
                )
                existing_email.recipient = message_data.get(
                    "recipient", existing_email.recipient
                )
                existing_email.body_text = message_data.get(
                    "body_text", existing_email.body_text
                )
                existing_email.body_html = message_data.get(
                    "body_html", existing_email.body_html
                )
                existing_email.attachments = message_data.get(
                    "attachments", existing_email.attachments
                )
                existing_email.headers = message_data.get(
                    "headers", existing_email.headers
                )
    ...
    def get_emails(
        ...
        try:
            ...
            # 정렬 및 페이징
            emails = (
                query.order_by(Email.date_received.desc())
                .offset(offset)
                .limit(limit)
                .all()
            )
