...
class ChatBotConfig:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_prompt": (
                "당신은 ProxyMe라는 도움이 되는 AI 어시스턴트입니다. " "사용자의 질문에 정확하고 유용한 답변을 제공해주세요."
            ),
        }
    ...
    def _load_env_config(self):
        ...
        # LiteLLM 설정
        if os.getenv("LITELLM_HOST_URL"):
            self.config["llm"]["litellm_host"] = os.getenv("LITELLM_HOST_URL")

        if os.getenv("LITELLM_API_KEY"):
            self.config["llm"]["litellm_api_key"] = os.getenv("LITELLM_API_KEY")
